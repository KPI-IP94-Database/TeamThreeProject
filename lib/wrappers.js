...
/**
 * Database.get(table, field, value)
 * table - name of the table, string
 * field - name of the field to check, string
 * value - needed value to check, typed
 * callback - callback, that takes the result, function
 * Passes the whole row that contains the needed value.
 *
 * Example of the use:
 * ```
 * const e-mail = 'pupkin@gmail.com';
 * const user = db.get('user', 'email', e-mail, exists => {
 *   db.insert('user', [e-mail, 'password', 100, 100, 100, 9.8]);
 * });
 * ```
 */

Database.prototype.get = function(table, field, value, callback) {

  const wrappedValue = wrapValue(table, field, value);

  this.get(`SELECT EXISTS(SELECT * FROM ${table}
            WHERE ${field}=${wrappedValue});`, (err, rows) => {
    if (err) throw err;
    const result = rows[0];
    callback(result);
  });
