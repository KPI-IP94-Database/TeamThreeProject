...
const dbObjs = require('./dbObjects.js');
...
Database.prototype.insertObj = function(obj) {
  if (obj instanceof dbObjs.Student) {
    this.insert('user', [obj.email, obj.password, obj.grades[0], obj.grades[1], obj.grades[2], obj.certificate]);
  } else if (obj instanceof dbObjs.Admin) {
    this.insert('admin', [obj.email, obj.password]);
  } else if (obj instanceof dbObjs.Application) {
    this.insert('application', [obj.userId, obj.specialityId, obj.priority]);
  } else if (obj instanceof dbObjs.Speciality) {
    this.insert('faculty', [obj.name, obj.facultyId, obj.budgetPlaces, obj.commercePlaces]);
 ...
 * Passes true if the value if already inside table.
 *
 * Example of the use:
 * ```
