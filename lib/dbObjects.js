...
const { TypeException } = require('./errors.js');

//throwTypeException
const throwTE = (obj, expectedType = '') => {
  const expected = expectedType === '' ? '' : 'Expected ' + expectedType;
  throw new TypeException('Invalid Type ' + obj.constructor.name +
    ', expected ' + expected);
};
...
Database.prototype.insertObj = function(obj) {
  ...
  if (obj instanceof Student)
    ...
    this.insert('specialty', [obj.name, obj.facultyId,
...
Database.prototype.removeObj = function(obj) {
  ...
  if (obj.id != undefined && obj.id >= 0) {
    ...
    this.removeRow(table, 'id', obj.id);
  }
  else throwTE(obj);
